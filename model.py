...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        if self.flash:
            ...
            if attn_mask is not None:
                attn_mask = attn_mask.reshape((B, 1, 1, T))
                att = att.masked_fill(attn_mask == 0, float('-inf')) # pad the keys 
                # We dont pad the queries because a pad token can only query anohter pad token
